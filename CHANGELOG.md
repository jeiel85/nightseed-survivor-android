...
## v0.36.0 - 2026-06-11 (난이도 리워크와 화면 통일)

### Changed — 난이도 리워크

- 적 재배치 신규: 플레이어에게서 1050px 이상 떨어진 적을 진행 방향 앞쪽 스폰 링으로 재배치해 circle-kiting 무한 생존 루트를 차단했습니다 (보스/미니보스 제외).
- Normal 난이도 배수를 0.85/0.9 → 1.0/1.0으로 (적 약화 보정 제거), Hard 1.7/1.45, Nightmare 2.9/2.0으로 올렸습니다.
- 적 시간 스케일을 분당 HP +34% / DMG +20% / SPD +8%로 상향했습니다.
- 피격 무적을 0.5초 → 0.35초로, 무기 레벨업 곡선을 DMG ×1.22 / CD ×0.90으로 조정했습니다.
- 봇 측정 하니스 `tests/sim_afk_run.gd`를 추가했습니다. 측정 결과는 docs/BALANCE.md §12 참고.

### Changed — 하위 화면 UI 통일

- `UIKit.gd` 공용 크롬을 신설하고 상점/캐릭터/스테이지/설정 화면에 적용했습니다 — 밤하늘 백드롭, 금색 타이틀, 석판 카드, 선택 카드 금색 글로우, 단일 텍스트 팔레트.

### Fixed

- 레벨업 카드 등급 글로우가 한 번도 렌더링되지 않던 버그를 수정했습니다 (존재하지 않는 API 호출로 매번 스킵).
- 카드 텍스트가 테두리 장식·글로우 프레임과 겹치던 증상을 수정했습니다 — 석판 텍스처 투명 거터 크롭 + 글로우를 draw 시그널 직접 드로잉으로 교체.

### Build / CI

- Android / Android AAB `versionCode`를 40 → 41로, `versionName`을 0.35.0 → 0.36.0으로 올렸습니다.

## v0.35.0 - 2026-06-11 (전투 UI 폴리시와 앱 용량 다이어트)

### Added — UI 자산 연결 (Phase UI-5 완료)

- 결과 화면: 승리 시 트로피 현수막 안에 타이틀 표시, 석판 패널 적용, 획득 골드를 코인 아이콘 + 금색 전용 행으로 강조했습니다. 패배는 붉은 타이틀을 유지해 승/패가 한눈에 구분됩니다.
- HUD: 남은 시간 옆 시계 아이콘을 추가하고, 처치 카운트 아이콘을 임시 단검에서 전용 해골 아이콘으로 교체했습니다.
- 가상 조이스틱: 코드 원 드로잉을 달빛 링/썸 텍스처로 교체했습니다 (텍스처 미임포트 시 기존 절차적 드로잉 fallback).
- 메인메뉴: 로고 뒤 달빛 덩굴 장식, 설정 버튼 기어 아이콘을 연결했습니다.

### Changed — 자산 최적화 (13.4MB → 2.3MB)

- 스토리 인장 5종·책 아이콘을 1254×1254 → 256×256으로, 봉인 체인을 1024×256으로 다운스케일했습니다. 풀스크린 배경 3종은 치수 유지 + 256색 팔레트 양자화로 재압축했습니다.
- 미참조 레거시 `shop_warriors_might.png`를 삭제했습니다.
- `docs/ASSET_GUIDE.md`를 현행화하고 "표시 크기 3배 초과 원본 커밋 금지" 규율을 박제했습니다.

### Build / CI

- Android / Android AAB `versionCode`를 39 → 40으로 올렸습니다.
- Android / Android AAB `versionName`을 0.34.1 → 0.35.0으로 올렸습니다.
- 키스토어 경로를 `secrets/` → `.keystore/`로 정리했습니다.

### Verification

- `tests/verify_ui_wiring.gd` headless 검증 ALL OK (씬 로드 + 노드 경로 + 텍스처 로드).
- `tests/screenshot_result_panel.gd` 승리/패배 결과 화면 캡처 확인. 다운스케일 자산 시각 품질 열화 없음.
