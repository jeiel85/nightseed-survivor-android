...
봇 측정 하니스로 "자동사냥" 원인을 특정(circle-kiting + 상한 악용)하고 적 재배치로 차단. 난이도 수치 일괄 상향. UIKit 공용 크롬으로 하위 4개 화면 통일. 카드 텍스트 겹침(텍스처 거터 + PanelContainer 자식 강제 배치) 수정. v0.36.0 (vc41) 릴리즈.

### Next

- 실기 체감 난이도 검증 (과하면 EnemyBase 시간 스케일 계수 롤백)
- 레벨업 카드 등급 프레임 실기 확인 (이번에 처음 표시)
